...
import { readableStreamToString, YnabError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new YnabError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new YnabError(e, e.response?.data)
                }
                if (innerError instanceof YnabError) {
                    // Got "YnabError" from the above try block
                    throw innerError;
                }
